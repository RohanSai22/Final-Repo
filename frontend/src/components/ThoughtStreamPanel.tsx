...
import { BackgroundGradient } from "./ui/background-gradient"; // Assuming aceternity-ui components are in ui folder
...
export const ThoughtStreamPanel: React.FC<ThoughtStreamPanelProps> = ({
  ...
  return (
    <BackgroundGradient
      className={`w-full max-w-sm mx-auto rounded-xl p-0.5 ${className}`} // Gradient applied to border via padding trick
      containerClassName="w-full" // Ensure container takes full width for proper gradient sizing
      animate="hover"
    >
      <div className="bg-neutral-900 backdrop-blur-sm rounded-[10px] text-neutral-200 w-full"> {/* Solid bg, less opacity for inner if needed */}
        <button
      ...
      >
        <div className="flex items-center">
          ...
          <h3 className="text-xs font-medium tracking-wide uppercase">{panelTitle}</h3>
        </div>
        <Button variant="ghost" size="sm" className="text-neutral-400 hover:text-neutral-100 p-1 h-auto">
      ...
      {!isCollapsed && (
        ...
        >
          {stream.length === 0 && !isThinking && (
            <p className="text-xs text-neutral-500 italic text-center py-6">No thoughts to display.</p>
          )}
