...
import {
  Lightbulb, Brain, Zap, ClipboardCheck, BarChart2,
  ListChecks, MapIcon, Search, Globe, Network, Terminal, Code2,
  PenTool, Activity, ChevronRight
} from 'lucide-react';
...
type MappedIconType = 'hypothesis' | 'action' | 'analysis' | 'strategy' | 'search_queries' | 'web_research' | 'reflection' | 'code_execution' | 'finalizing_answer';

const iconMap: Record<MappedIconType, React.ElementType> & { default: React.ElementType } = {
...
export const CognitiveBlock: React.FC<CognitiveBlockProps> = ({ block, isLast, isFirst }) => {
  const IconComponent = (iconMap[block.type as MappedIconType] || iconMap.default);
  ...
  const renderDetails = (blockData: CognitiveBlockData) => {
    ...
    if (blockData.type === 'code_execution' && typeof details === 'object' && details !== null && ('code' in details || 'output' in details)) {
      ...
      return (
        <div className="mt-1.5 space-y-1.5">
          {code && (
            <div>
              <p className="text-xs font-medium text-neutral-400 mb-0.5">Executed Code:</p>
          ...
          {output && (
            <div>
              <p className="text-xs font-medium text-neutral-400 mb-0.5">Output:</p>
      ...
      return (
        <div className="flex flex-wrap gap-1 mt-1.5">
          {details.map((item, index) => (
            <Badge key={index} variant="secondary" className="text-xs bg-neutral-700 text-neutral-300 border-neutral-600 px-1.5 py-0.5 font-normal">
              {item}
            </Badge>
          ))}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-0 top-4 transform -translate-x-0 flex items-center justify-center"> {/* Adjusted left to align with new pl */}
      ...
      {/* Content Area */}
      <div className="ml-6 p-3 rounded-lg bg-neutral-800 border border-neutral-700/60 shadow-sm hover:shadow-lg transition-shadow duration-200 ease-in-out"> {/* Increased padding, rounded-lg, hover:shadow-lg */}
        {block.content.title && (
          <h4 className="text-xs font-semibold text-neutral-200 mb-0.5 flex items-center">
            {block.content.title}
          </h4>
        )}
        <p className="text-xs text-neutral-300 leading-relaxed min-h-[1.5em]">{animatedText}</p>
        {renderDetails(block)}
        {block.timestamp && (
           <p className="text-[10px] text-neutral-500/80 mt-1.5 pt-1 border-t border-neutral-700/50 text-right"> {/* Lighter timestamp */}
